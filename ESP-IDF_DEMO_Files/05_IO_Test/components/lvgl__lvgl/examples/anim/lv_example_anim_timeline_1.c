...
{
    /* obj1 */
    lv_anim_t a1;      //定义一个 lv_anim_t 类型的变量
    lv_anim_init(&a1); //初始化 a1 动画变量
    lv_anim_set_var(&a1, obj1); //设置动画的目标对象
    lv_anim_set_values(&a1, 0, obj_width); //设置动画的起始和结束值
    lv_anim_set_early_apply(&a1, false); //设置是否立即应用动画的起始值
    //设置一个回调函数，该函数将在每个动画帧执行时被调用
    lv_anim_set_exec_cb(&a1, (lv_anim_exec_xcb_t)set_width); 
    //设置动画的路径函数
    lv_anim_set_path_cb(&a1, lv_anim_path_overshoot);
    lv_anim_set_time(&a1, 300);//设置动画的总持续时间，以毫秒为单位
