...
def test_Eyal_espresso_ESP32(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
    dut.expect('Eyal_espresso_ESP32!')
    log_minimum_free_heap_size()


@pytest.mark.host_test
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_Eyal_espresso_ESP32_linux(dut: IdfDut) -> None:
    dut.expect('Eyal_espresso_ESP32!')
...
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_Eyal_espresso_ESP32_macos(dut: IdfDut) -> None:
    dut.expect('Eyal_espresso_ESP32!')
...
def test_Eyal_espresso_ESP32_host(app: QemuApp, dut: QemuDut) -> None:
