...
$bodyItems = @(
    ...
    "v0.3.6 - DATA path refresh and deterministic Simulate Data plotting"
    "  - Client downlink FIFO depth increased to 64 packets"
    "  - FIFO full behavior changed from drop to wait (delay/back-pressure over client-side data loss)"
    "  - Simulate Data UI poll period reduced from 100 mSec to 20 mSec"
    "  - Simulate Data UI now consumes one packet per poll tick to preserve FIFO order and avoid burst-collapse rendering"
    ""
    ...
    "  Low-Level Controller -> enqueue validated downlink payload into client DATA FIFO"
    "  FIFO policy -> if full, enqueue waits for space (delay/back-pressure), no client-side drop at FIFO boundary"
    "  UI path -> poll every 20 mSec and consume one packet per tick in FIFO order"
    "  UI path -> update Simulate Data graph once per consumed packet"
    "  Peer -> DATA_ACK or application response"
    "  Upper Layer -> consume payload only after low-level validation and FIFO ordering"
    "[[IMAGE|packet_data_flow.png|Figure 8. DATA packet flow showing low-level validation before application payload handling.]]"
    ""
    "5.6A Implemented Client DATA Path (Simulate Data)"
    "Implemented Packet Source:"
    "  Simulator sends one binary downlink packet at 100 mSec intervals when DataSimulation is enabled."
    "Implemented Packet Shape:"
    "  magic(0xD0), seq(uint32), 100 floats, 20 ints, 50-byte string."
    "Implemented Buffering and Rendering Chain:"
    "  CommunicationFunctions RX -> data_downlink_push() -> downlink FIFO (depth 64) -> ui_sim_data_poll_timer_cb() -> data_downlink_pop() -> ui_plot_sim_data_packet()."
    "Determinism Rule:"
    "  The UI pops one packet per 20 mSec tick, preserving FIFO order and avoiding visual collapse of multiple arrivals into one redraw."
    "Loss vs Delay Rule:"
    "  At the client FIFO boundary, overload is represented as queueing delay rather than packet drop."
    "Observed Tradeoff:"
    "  Under prolonged overload, latency can grow because producer enqueue waits for available FIFO slots."
    ""
    "5.7 ERROR Packet"
